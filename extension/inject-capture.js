(() => {
  ...
  function colorToFigma(color) {
    ...
    return { r: r / 255, g: g / 255, b: b / 255, a: Number.isFinite(a) ? a : 1, css: color };
  }
  ...
  function estimateLayoutMode(style) {
    ...
    if (style.display === 'flex') return style.flexDirection && style.flexDirection.startsWith('column') ? 'VERTICAL' : 'HORIZONTAL';
  ...
  function getElementName(el, tag) {
    return safeName(
      el.getAttribute('data-testid') || el.getAttribute('aria-label') || el.getAttribute('name') || el.id || el.className,
      tag,
    );
  }
  ...
  function captureFromRootElement(rootElement, options = {}) {
    ...
    const walkOptions = { selector: options.selector || null, rootElement };
